...
const formSchema = z.object({
    ...
    value: z.string().min(4).regex(/^#/, {
        message: 'Строчка должна быть значением hex code'
    })
})
...
export const ColorForm: React.FC<ColorFormProps> = ({
    ...
    const title = initialData ? "Изменить цвет" : "Создать цвет";
    ...
    const onDelete = async () => {
        try {
            ...
            toast.error('Сначала убедитесь, что вы удалили все продукты для этого цвета?')
        } finally {
            setLoading(false)
            setOpen(false)
        }
